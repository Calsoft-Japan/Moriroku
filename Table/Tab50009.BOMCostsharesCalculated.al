table 50009 "BOM Cost shares Calculated"
{
    ...
    fields
    {
        ...
        { }
        field(50003; "ValueEntry Total Cost"; Decimal)
        {
            Caption = 'Total Value Entry Cost';
        }
        field(50004; "Line Allocate Cost"; Decimal)
        {
            Caption = 'Allocation Amount';
        }
        field(50005; "GL Acct."; Code[20])
        {
            Caption = 'COGS Allocation Account';
        }
        field(50006; "G/L Account Cost"; Decimal)
        { }
        field(50007; "Entry Type"; Option)
        {
            OptionMembers = Sales,Scrap,Adjustment,Revalue;
        }
        /* field(50008; "Total Cost per Item"; Decimal)
        {
    ...
    var
        ParentBOMBuffer: Record "BOM Cost shares Calculated";
    ...
    var
        Item: Record Item;
        CopyOfBOMBuffer: Record "BOM Cost shares Calculated";
    ...
    local procedure OnAfterCalcUnitCost(var BOMBuffer: Record "BOM Cost shares Calculated")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterGetItemCosts(var BOMBuffer: Record "BOM Cost shares Calculated"; Item: Record Item);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitFromItem(var BOMBuffer: Record "BOM Cost shares Calculated"; Item: Record Item; StockkeepingUnit: Record "Stockkeeping Unit");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitFromRes(var BOMBuffer: Record "BOM Cost shares Calculated"; Resource: Record Resource);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitFromMachineCenter(var BOMBuffer: Record "BOM Cost shares Calculated"; MachineCenter: Record "Machine Center");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitFromWorkCenter(var BOMBuffer: Record "BOM Cost shares Calculated"; WorkCenter: Record "Work Center");
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterIsLineOk(var BOMBuffer: Record "BOM Cost shares Calculated"; LogWarning: Boolean; var BOMWarningLog: Record "BOM Warning Log"; var Result: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterRoundCosts(var BOMBuffer: Record "BOM Cost shares Calculated"; ShareOfTotalCost: Decimal);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterTransferFromProdComp(var BOMBuffer: Record "BOM Cost shares Calculated"; ProductionBOMLine: Record "Production BOM Line"; ParentItem: Record Item; var EntryNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcOvhdCost(var BOMBuffer: Record "BOM Cost shares Calculated"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInitFromItem(var BOMBuffer: Record "BOM Cost shares Calculated"; Item: Record Item; var IsHandled: Boolean);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsBOMOk(var BOMBuffer: Record "BOM Cost shares Calculated"; var Result: Boolean; var IsHandled: Boolean; var LogWarning: Boolean; var BOMWarningLog: Record "BOM Warning Log")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsLowLevelOk(var BOMBuffer: Record "BOM Cost shares Calculated"; var Result: Boolean; var IsHandled: Boolean; var LogWarning: Boolean; var BOMWarningLog: Record "BOM Warning Log")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInitFromRes(var BOMBuffer: Record "BOM Cost shares Calculated"; Resource: Record Resource; var IsHandled: Boolean);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInitFromMachineCenter(var BOMBuffer: Record "BOM Cost shares Calculated"; MachineCenter: Record "Machine Center"; var IsHandled: Boolean);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnbeforeInitFromWorkCenter(var BOMBuffer: Record "BOM Cost shares Calculated"; WorkCenter: Record "Work Center"; var IsHandled: Boolean);
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateAbleToMake(var BOMBuffer: Record "BOM Cost shares Calculated"; var AvailQty: Decimal; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnGetItemCostsOnBeforeRoundCosts(var BOMBuffer: Record "BOM Cost shares Calculated")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromItemCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; Item: Record Item)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromAsmHeaderCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; AssemblyHeader: Record "Assembly Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromAsmLineCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; AssemblyLine: Record "Assembly Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromBOMCompCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; BOMComponent: Record "BOM Component")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromProdCompCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; ProductionBOMLine: Record "Production BOM Line"; ParentItem: Record Item; ParentQtyPer: Decimal; ParentScrapQtyPer: Decimal)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromProdOrderLineCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; ProdOrderLine: Record "Prod. Order Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromProdOrderCompCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; ProdOrderComponent: Record "Prod. Order Component")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnTransferFromProdRoutingCopyFields(var BOMBuffer: Record "BOM Cost shares Calculated"; RoutingLine: Record "Routing Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsQtyPerOk(var BOMBuffer: Record "BOM Cost shares Calculated"; var BOMWarningLog: Record "BOM Warning Log"; LogWarning: Boolean; var Result: Boolean; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeIsReplenishmentOk(var BOMBuffer: Record "BOM Cost shares Calculated"; var BOMWarningLog: Record "BOM Warning Log"; LogWarning: Boolean; var Result: Boolean; var IsHandled: Boolean)
    begin
    end;
